...
// Check if the session contains email and password
...
$conn = new mysqli($servername, $username, $password, $dbname);
    ...
    // Get the playlist_id from the URL
    if (isset($_GET["playlist_id"])) {
        $playlist_id = $_GET["playlist_id"];
        
        // Query the playlist_song table for playlist songs
        $sqlSongs = "SELECT s.song_id, s.title, s.artist FROM songs s
                     INNER JOIN new_playlist_song ps ON s.song_id = ps.song_id
                     WHERE ps.playlist_id = $playlist_id AND ps.email_id = '$email'";
        
        $resultSongs = $conn->query($sqlSongs);

        // Fetch the playlist name using the provided playlist_id
    $query = "SELECT name FROM playlist WHERE playlist_id = '$playlist_id' AND email_id = '$email'";
...
<body>
        ...
        <ul>
            ...
            <li>
                <form class="search-form" action="/search" method="GET">
                    ...
                    <input type="text" name="query" placeholder="liked songs" />
    ...
    <?php if (isset($songs) && count($songs) > 0) : ?>
        <?php if (isset($playlist_name)) : ?>
    <h1 class="playlist_name"><?php echo $playlist_name; ?></h1>
<?php endif; ?>
...
<?php else : ?>
    <div class="container">
        <div class="no-results">
            <div class="no-results-item">No songs found in this playlist.</div>
            <div class="no-results-item">Explore more songs:</div>
