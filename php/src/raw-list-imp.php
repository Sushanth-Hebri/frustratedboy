...
<script>// Declare anotherVariable in the global scope
var anotherVariable;
...
function fetchSongDetails() {
    ...
    $url = "http://localhost:8000/get_song_details.php?album_id=" . urlencode($albumId);
    $response = file_get_contents($url);

    // Assuming the response is in JSON format, you can decode it to an array
    ...
    $albumId = '<?php echo $albumId; ?>';
    $songDetails = fetchSongDetails();

    echo '<div id="songList">';

    foreach ($songDetails as $index => $song) {
        $activeClass = ($index === 0) ? ' active' : '';
        echo '<div class="outer' . $activeClass . '" data-song-id="' . $song['song_id'] . '">';
        echo '<div class="left" onclick="redirectToSinglePlayer(' . $song['song_id'] . ')">';
        echo '<img src="' . $song['song_id'] . '.jpg" alt="like" class="leftpng">';
        echo '</div>';
        echo '<div class="right">';
        echo '<div class="title" onclick="playSongsus(' . $song['song_id'] . ', this)">';
        echo '<h1>' . $song['title'] . '</h1>';
        echo '</div>';
        echo '<div class="count">';
        echo '<h6>' . $song['artist'] . '</h6>';
        echo '</div>';
        echo '</div>';
        echo '<div class="option">';
        echo '<span class="material-symbols-outlined more">more_vert</span>';
        echo '<div class="dropdown-menu">';
        echo '<ul>';
        echo '<li>';
        echo '<div class="like-button" data-song-id="' . $song['song_id'] . '">';
        echo '<ion-icon name="heart"></ion-icon>';
        echo '</div>';
        echo '</li>';
        echo '<li class="addToPlaylist" data-song-id="' . $song['song_id'] . '">Add to playlist</li>';
        echo '</ul>';
        echo '</div>';
        echo '</div>';
        echo '</div>';
...
console.log('Selected classes:', optionElements.className);
// Iterate through each 'option' element
optionElements.forEach(optionElement => {
    // Add a click event listener to each 'option' element
    optionElement.addEventListener('click', () => {
        // Find the 'dropdown-menu' element within the clicked 'option' element
        const dropdownMenu = optionElement.querySelector('.dropdown-menu');
        ...
        if (dropdownMenu) {
            ...
            // Log the selected classes to the console
...
// console.log(options); // Check if the correct elements are selected

//         options.forEach(function(option) {
//             option.addEventListener('click', function() {
                
//                 console.log('Dropdown clicked'); // Add this line to check if the event listener is working
